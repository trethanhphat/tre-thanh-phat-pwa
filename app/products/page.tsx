...
import { getImageURL, saveImageIfNotExists } from '@/lib/images';
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // ✅ Cache ảnh sau khi sync
        for (const p of cached) {
          if (p.image_url) {
            await saveImageIfNotExists(p.image_url);
            const localUrl = await getImageURL(p.image_url);
            setImageMap(prev => ({ ...prev, [p.id]: localUrl }));
          }
        }
